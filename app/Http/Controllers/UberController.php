...
class UberController extends Controller
{
    ...
    public function estimate_fare(Request $request){
        ...
        Log::info('Receive estimation fare request'.json_encode($all_input));
        $start_latitude = $request->input('lat_start');
        $start_longitude = $request->input('long_start');
        $end_latitude = $request->input('lat_end');
        $end_longitude = $request->input('long_end');
        ...
        curl_setopt($ch, CURLOPT_URL, "https://api.uber.com/v1.2/estimates/price?start_latitude=".$start_latitude."&start_longitude=".$start_longitude."&end_latitude=".$end_latitude."&end_longitude=".$end_longitude."");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");

        $headers = array();
        $headers[] = "Authorization: Token rb-mqT6FLxxHN_wDkowj4Dr3YKXydhJ45vo8ex00";
        ...
        $estimation['estimation'] = $data->estimate;
        $estimation['destination'] = $destination; 
        
        Log::info('Sent Detail Estimation fare to User');
        return response()->json(['result_code' => 1, 'result_message' => 'Request estimation ride fare from Uber API success', 'data' => $estimation]);
    ...
    public function detail_request($request_id){
        $ch = curl_init();
          ...
          if (curl_errno($ch)) {
              echo 'Error:' . curl_error($ch);
              Log::error('Failed to send detail current request to Uber API');
          }
          curl_close ($ch);
          print_r($result);
    }
